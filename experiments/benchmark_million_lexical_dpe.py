"""Disk-backed one-million-document lexical-DPE benchmark.

This closes the gap between the HMAC-FTS5 scale benchmark and the full lexical
path executed on small corpora.  Candidate generation reads the complete union
of the two rarest HMAC term postings (up to a declared safety cap) without
using FTS5's BM25 order.  The server then ranks signed feature-hashed BM25
MIPS-to-L2 vectors under stored conditional-DPE ciphertexts, returns top-200,
and the client reranks those documents using exact un-hashed BM25.

The construction is resumable.  NumPy PCG64 models the bounded-noise PRF and is
not a production cryptographic PRF, exactly as in the semantic prototype.
"""
