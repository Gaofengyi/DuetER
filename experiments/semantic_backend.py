"""Streaming semantic backend used by the complete-corpus DuetER experiment.

The module provides corpus selection, Granite embedding, residual-IVF storage,
CCADPE query transformation, ranking, metrics, and atomic artifact helpers.
Large arrays are memory-mapped so the final NQ, HotpotQA, and MS MARCO runs can
resume without rebuilding completed stages. NumPy PCG64 is used only for
deterministic research experiments and is not a production PRF.
"""
...
from common import fwht_batch, normalize_rows, tokenize
