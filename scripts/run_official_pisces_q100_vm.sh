...
run_path() {
  ...
  /usr/bin/time -v -o "${OUT}/${path}_fiqa_n57638_q100_k${K}.time" \
    "${binary}" \
    --database_path="${corpus}" \
    --query_path="${query}" \
    --output_file="${OUT}/${path}_fiqa_n57638_q100_k${K}.jsonl" \
    --K="${K}" \
    > "${OUT}/${path}_fiqa_n57638_q100_k${K}.log" 2>&1
  test "$(wc -l < "${OUT}/${path}_fiqa_n57638_q100_k${K}.jsonl")" -eq 100
  grep -E 'Elapsed|Maximum resident|Exit status' "${OUT}/${path}_fiqa_n57638_q100_k${K}.time"
